```console
$ docker run --rm dcm4che/oauth2-proxy:6.1.1 --help
Usage of oauth2-proxy:
      ...
      --keycloak-group string                      restrict login to members of this group.
      ...
      --skip-auth-regex strings                    bypass authentication for requests path's that match (may be given multiple times)
      --skip-auth-strip-headers                    strips X-Forwarded-* style authentication headers & Authorization header if they would be set by oauth2-proxy for request paths in --skip-auth-regex
      ...
      --user-id-claim string                       which claim contains the user ID (default "email")
